...
async def ws_parking(websocket: WebSocket) -> None:
    """Stream parking session open/close events across all sites."""
    import redis.asyncio as aioredis
    from drishtiai_api.config import settings
    ...
    r = aioredis.from_url(settings.redis_url, decode_responses=True)
    pubsub = r.pubsub()
    await pubsub.psubscribe("drishti:*:parking")
    ...
    try:
        ...
        await pubsub.punsubscribe("drishti:*:parking")
        await pubsub.aclose()
        await r.aclose()
